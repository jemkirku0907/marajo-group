...
(async () => {
  ...
  try {
    ...
    await client.query("ALTER TABLE staff ADD COLUMN IF NOT EXISTS company_code TEXT NOT NULL DEFAULT 'marajo_group'");
    await client.query("UPDATE staff SET company_code='marajo_group' WHERE company_code IS NULL OR company_code=''");
    const existing = await client.query("SELECT id FROM staff WHERE email=$1 LIMIT 1", [email]);
    if (existing.rows[0]) {
      await client.query(
        `UPDATE staff
         SET name=$1, role=$2, role_code=$3, password_hash=$4, is_active=1, company_code='marajo_group'
         WHERE email=$5`,
      ...
      await client.query(
        `INSERT INTO staff (name, role, role_code, email, password_hash, is_active, company_code)
         VALUES ($1,$2,$3,$4,$5,1,'marajo_group')`,
